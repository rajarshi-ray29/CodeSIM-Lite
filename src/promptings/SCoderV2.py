...
class SCoder(DirectStrategy):
    ...
    def run_single_pass(self, item: dict):
        ...
        for try_no in range(1, self.k+1):
            input_planning_simulation = [
                {
                    "role": "user",
            ...
            print("\n\n________________________")
            print(f"Input for Planning and simulation: {try_no}")
            print(input_planning_simulation[0]['content'], flush=True)

            response, pr_tok_1, com_tok_1 = self.gpt_chat(
                processed_input=input_planning_simulation
            )
            ...
            print("\n\n________________________")
            print(f"Response from Planning and simulation: {try_no}")
            print(response, flush=True)
            ...
            input_for_final_code_generation = [
                {
                    "role": "user",
                    "content": f"Given a competitive programming problem, plan, input to output simulation, generate {self.language} code to solve the problem.\n\n{problem_with_planning}\n\n##  Code:\n\n----------------\nImportant:\n{std_input_prompt}\n## Solve the problem step by step by following every steps. Your response must contain only the {self.language} code to solve this problem."
                }
            ]

            print("\n\n________________________")
            print(f"Input for final code generation: {try_no}")
            ...
            print("\n\n________________________")
            print(f"Response from final code generation: {try_no}")
...
----------------
Instructions: 
- Suppose you are a tester, your task is to create comprehensive test cases for a programming problem.
- The test cases must contain both **Normal** and **Edge** case scenarios to ensure the code's correctness.
- Do not generate more than 20 test cases.
- Your response must contain the generated test cases inside ``` block."""


prompt_for_code_generation = """# Problem
{problem}
...
## TODO: Generate Problem Understanding, Plan and Simulation.

----------------
Instructions: 
- Suppose you are a programming expert. By seeing a problem you first understand the problem then generate a plan and finally apply the plan in sample input to get the sample output. 
- Give a paragraph containing problem description followed by a plan to solve the programming problem. Finally, the simulation of sample input and output. 
- Follow the style shown in the given example problem.
- Your response must contain only three sections. They are **Problem Understanding**, **Plan** and **Simulation**.'''
...
----------------
Instructions: 
- Suppose you are a debbugging expert.
- You have a programming problem along with its plan and sample input to output simlation.
- You also have a {language} code to solve the programming problem.
- Unfortunately, the generated {language} code failed to pass the test cases. Test Report was given before.
- Verify the plan and if necessary change the plan and generate {language} generate code accordingly.
- Your response must contain the modified {language} code inside ``` block.
{std_input_prompt}"""
